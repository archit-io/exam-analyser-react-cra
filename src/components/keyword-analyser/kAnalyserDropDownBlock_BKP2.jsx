...
class KAnalyserDropdownBlock extends Component {
    ...
    genCurrDropDown = () => {
      ...
      const dropDownMenu = i[j].dropV.map((index) => (<Dropdown.Item  onClick={this.handleClick(index.eventK)} key={index.eventK} eventKey={index.eventK}>{index.val}</Dropdown.Item> ));
      return (
    ...
    mainDropDownBlock()
        {
            ...
            if (clickState === '') //DEFAULT STATE ON PAGE LOAD
                ...
                {return [this.genCurrDropDown(), this.genCurrDropDownId('mainsOptions'), this.genCurrDropDownId('prelimsOptions')];}             
